...
for i, (n, t) in enumerate(items):
    ...
    body += txt(t, x + 64, y + 13, 340, size=13, lh=1.35, color=C['ink'])
...
# ═══ 04 1 095 ═══
body = f'''
<div class="serif" style="position:absolute;left:52px;top:150px;font-weight:900;font-size:210px;line-height:1;color:{C['ink']}">1 095</div>
<div style="position:absolute;left:52px;top:388px;width:60px;height:5px;background:{C['accent']}"></div>
'''
body += p('приёмов пищи на жителя в год. Больше, чем любых других услуг дома. <b>Каждый из них — решение, которое кто-то принял за человека.</b>', 415, size=17, lh=1.55)
...
body += txt('Четыре из пяти жителей ПНИ не могут юридически заявить выбор — но выбирают взглядом, ложкой, отказом.', 380, 180, 500, size=16.5, lh=1.55, color=C['ink'])
body += txt('Задача системы — услышать этот выбор другими каналами: наблюдение, биография, «те, кто знает».', 380, 280, 500, size=14.5, lh=1.55, color=C['soft'])
body += p('Каналы воли — слайд 25. Правовая рамка — разделы 15–16 доклада.', 415, size=12.5, color=C['muted'])
...
body += hbar(['Швеция — ориентир', 'США — норма CMS', 'Типовой дом, РФ'],
             [11, 14, 15], 52, 160, 696, unit=' ч',
             colors=[C['gray'], C['soft'], C['accent']])
body += p('Ужин в 17:30 → завтрак в 8:30. Для жителей на антипсихотиках голодная ночь — это вес, поведение и ночные «набеги» на кухню.', 350, size=15, lh=1.55)
body += box(52, 412, 856, 52, C['accentsoft'])
body += txt('Вопрос залу: какая цифра в вашем доме? Замер — три дня, одна строка в журнале, ноль рублей.', 74, 427, 820, size=13.5, bold=True, color=C['ink'])
...
for i, node in enumerate(chain):
    ...
    body += txt(node, x + 12, 180, 126, size=13, align='center', lh=1.4, color=C['ink'])
    if i < 3:
...
cards = [('442-ФЗ', '«учёт индивидуальной потребности» — выбор блюда укладывается в неё напрямую'),
         ('3185-1, ст. 37 и 43', 'достоинство и гуманное отношение — распространены на проживающих ДСО и ПНИ'),
         ('СанПиН · обе редакции', 'не менее 3 приёмов, диетическое по показаниям — число вариантов не ограничено')]
for i, (t, d) in enumerate(cards):
    x = 52 + i * 294
    body += box(x, 150, 274, 212, C['panel'])
    body += txt(t, x + 24, 176, 230, size=17, serif=True, color=C['accentdeep'])
...
for i, (name, reg, what) in enumerate(pts):
    ...
    body += f'<div style="position:absolute;left:{x+22}px;top:{y+22}px;width:11px;height:11px;border-radius:50%;background:{C["accent"]}"></div>'
    body += txt(f'<b>{name}</b> <span style="color:{C["muted"]};font-size:11.5px">· {reg}</span><br>{what}', x + 48, y + 15, 356, size=12.5, lh=1.4, color=C['ink'])
...
for i, f in enumerate(['фарфор вместо железных мисок', 'вилки и ножи на столе', 'самообслуживание: поднос и линия раздачи', 'тренировочная кухня — доступ круглосуточно']):
    body += txt('—  ' + f, 524, 202 + i * 33, 370, size=13.5, color=C['ink'])
...
layers = [('Право', '442-ФЗ, 3185-1, ГК 29/30: выбор — реализация прав, а не милость'),
          ('Процедура', 'кто и как фиксирует выбор: журнал, совет, опекун — роль каждого'),
          ('Культура', '«за него решили» → «у него спросили» — самый медленный и главный слой')]
for i, (t, d) in enumerate(layers):
    y = 148 + i * 88
    body += box(52, y, 856, 78, C['panel'])
    body += txt(t, 84, y + 24, 200, size=19, serif=True, color=C['accentdeep'])
    body += txt(d, 300, y + 16, 580, size=14.5, lh=1.5, color=C['ink'])
...
body += txt('Гречка ↔ рис ↔ пшено', 52, 170, 500, size=30, serif=True, color=C['ink'])
body += txt('Курица ↔ индейка. Треска ↔ хек.', 52, 226, 500, size=30, serif=True, color=C['accentdeep'])
body += txt('Таблицы замены в раскладках — существующий законный механизм: равноценная замена внутри группы продуктов.', 52, 300, 470, size=14.5, lh=1.55, color=C['soft'])
...
steps = [('1', 'Врач назначает рамку', 'стол/текстура/ограничения — поимённо, с пересмотром'),
         ('2', 'Внутри рамки — пары', 'диетический стол тоже имеет эквиваленты'),
         ('3', 'Журнал замен', 'выбор фиксируется — защита жителя и врача'),
         ('4', 'Пересмотр', 'статистика выбора → корректировка цикла раз в квартал')]
...
ch = [('Показ двух порций', 'выбор глазами и рукой'), ('Наблюдение за съеденным', 'что осталось — то и сигнал'),
      ('Пищевая биография', 'привычки всей жизни'), ('«Те, кто знает»', 'персонал, родные, опекун')]
for i, (t, d) in enumerate(ch):
    x = 84 + i * 208
    body += txt(f'<span class="serif" style="font-weight:800;font-size:26px;color:{C["accent"]}">{i+1}</span>', x, 176, 40)
    body += txt(f'<b>{t}</b><br><span style="color:{C["soft"]}">{d}</span>', x, 222, 180, size=12.5, lh=1.45, color=C['ink'])
...
for i, (t, d) in enumerate(steps):
    ...
    body += txt(t, x + 24, 174, 230, size=19, serif=True, color=C['accentdeep'])
...
items = [('Доклад «Не просто накормить»', '250 страниц: право, клиника, экономика, 45 приложений-шаблонов'),
         ...
         ('Реестр источников', '125 проверенных позиций — каждое число проверяемо')]
for i, (t, d) in enumerate(items):
    x = 52 + i * 220
    body += box(x, 150, 200, 222, C['panel'])
    body += txt(f'<b>{t}</b><br><br>{d}', x + 18, 174, 168, size=12.5, lh=1.55, color=C['ink'])
body += p('Всё открыто: репозиторий материалов — по QR после сессии.', 412, size=14.5, bold=True)
...
body += txt('Материалы сессии — доклад, калькулятор, чек-листы, реестр источников:<br><b>github.com/qlolp/seminar-variable-food-2026</b>', 80, 272, 800, size=15, lh=1.7, color=C['ink'])
